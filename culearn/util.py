...
class KerasMath:
    ...
    def pinball(p, x, q):
        ...
        :return: Pinball loss.
        """
        p = list(p) if isinstance(p, Iterable) else [p]
        p_lower = K.concatenate([K.ones_like(x) * K.constant(_) for _ in p])
        p_upper = K.ones_like(q) - p_lower
        x = K.repeat_elements(x, len(p), axis=-1)
        return K.mean(K.maximum(p_upper * (q - x), p_lower * (x - q)))
