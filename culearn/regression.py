...
class S2S(StrMixin):
    ...
    def transform(x: np.array, lookback: 'int > 0', horizon: 'int > 0'):
        ...
        def lookback_slider(step: int) -> range:
            return range(step, len(x) - lookback - horizon + 1 + step)

        def horizon_slider(step: int) -> range:
            return range(lookback + step, len(x) - horizon + 1 + step)

        def slide_series(series: np.array, steps: range, slider: Callable[[int], range]) -> np.array:
            return np.column_stack(tuple([series[slider(step)] for step in steps]))

        def slide(steps: range, slider: Callable[[int], range]) -> np.array:
            n_steps = len(steps)
            n_series = np.shape(x)[1]
            n_sample = len(x) - lookback - horizon + 1
            if n_sample <= 0 or n_steps == 0 or n_series == 0:
                return np.empty(0)

            result = np.empty((n_sample, n_steps, n_series), float)
            for i in range(n_series):
                result[:, :, i] = slide_series(x[:, i], steps, slider)

            return result

        x_past = slide(range(lookback), lookback_slider)
        x_future = slide(range(horizon), horizon_slider)
